...
mitigate_reweighing <- reweighing(privileged_groups = privileged_groups,
                                  unprivileged_groups = unprivileged_groups)
data_aif_transformed <- mitigate_reweighing$fit_transform(data_aif_train)

# Initializing binary label dataset metric class with the transformed dataset
metric_train <- binary_label_dataset_metric(data_aif_transformed, 
                                            privileged_groups = privileged_groups, 
                                            unprivileged_groups = unprivileged_groups)
# Accessing `Statistical Parity Difference` metric
