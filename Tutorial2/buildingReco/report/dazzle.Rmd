...
}else if (params$plot_type == 'dygraphs'){
  
  #Dynnamic time series plot with dygraphs
 df <- df %>% 
    select(Date, Category, UR) %>%
    tidyr::pivot_wider(id_cols = 'Date', names_from = 'Category', values_from = 'UR')
  
 dygraph(xts::xts(df, order.by = df$Date), main = "Unemployment Rate") %>% 
  dyRangeSelector(dateWindow = c("2005-01-01", "2021-09-30")) %>%
